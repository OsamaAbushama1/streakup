...
// === 1. Register Fonts ===
const amsterdamFontPath = path.join(
  process.cwd(),
  "public",
  "templates",
  "fonts",
...
if (fs.existsSync(amsterdamFontPath)) {
  registerFont(amsterdamFontPath, { family: "Amsterdam" });
...
if (fs.existsSync(regularFontPath)) {
  registerFont(regularFontPath, { family: "Playfair Display Regular" });
...
// === 3. Template Map ===
const templateMap: Record<GenerateCertificateOptions["rank"], string[]> = {
  Silver: ["silver.png", "silver.jpg"],
  Gold: ["gold.png", "gold.jpg"],
  Platinum: ["platinum.png", "platinum.jpg"],
};

const getTemplatePath = (rank: GenerateCertificateOptions["rank"]): string => {
  ...
  throw new Error(
    `Template not found: public/templates/certificates/${files.join(" or ")}`
  );
};

// === 4. Format Date ===
const formatEnglishDate = (date?: string): string => {
...
export const generateCertificate = async ({
  ...
  rank,
  date,
}: GenerateCertificateOptions): Promise<Buffer> => {
  try {
    ...
    while (
      ctx.measureText(formattedName).width > maxTextWidth &&
      nameFontSize > 70
    ) {
      nameFontSize -= 5;
      ctx.font = `${nameFontSize}px 'Amsterdam', serif`;
    ...
    ctx.fillText(
      `Issued on: ${formatEnglishDate(date)}`,
      centerX - maxTextWidth / 2 + 20,
      descY + 100
    );
